...
instance Eq a => Eq (BTree a) where
	...
	t == t' = subEqual t t' && subEqual t' t where
		...
		subEqual (Node a l r) tree = subEqual l tree && subEqual r tree && elemEqual a tree where
			elemEqual _ Empty = False
			elemEqual x (Node a l r) = x == a || elemEqual x l || elemEqual x r
