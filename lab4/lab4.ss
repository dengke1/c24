...
(define (scalar-mult k m)
  (map (λ (v) (scalar-vector-mult k v)) m))
...
(define (transpose-rec m)
  (if (empty? (first m))
  '()
  (cons (map (λ (v) (first v)) m)
        (transpose-rec (map (λ (v) (rest v)) m) ))))
...
; return the matrix multiplication94 m1xm2
; no recursion: use maps, lambda expressions and the above
; transpose functions
(define (mult m1 m2)
  (map (λ (v) (map (λ (x) (dot-product x v)) (transpose m2))) m1))
